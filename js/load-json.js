...
function loadAnime() {
    ...
    fetch(userUrl, options).then(handleResponse)
        .then(handleData)
        .then(function(json) {
            const animeStats = json.data.User.statistics.anime;
            const mangaStats = json.data.User.statistics.manga;

            var animeWatched = animeStats.count;
            var episodesWatched = animeStats.episodesWatched;
            var avgAnimeScore = (animeStats.meanScore/10).toFixed(2);


            var mangaRead = mangaStats.count;
            var chaptersRead = mangaStats.chaptersRead;
            var avgMangaScore = (mangaStats.meanScore/10).toFixed(2);

            var html = '<div class="anime-stat">' + animeWatched + '</div>';
                html += '<div class="anime-stat">' + episodesWatched + '</div>';
                html += '<div class="anime-stat">' + avgAnimeScore + '</div>';
                html += '<div>anime watched</div>';
                html += '<div>episodes watched</div>';
                html += '<div>average score</div>';
            $('#anime-stats').append(html);

                html = '<div class="manga-stat">' + mangaRead + '</div>';
                html += '<div class="manga-stat">' + chaptersRead + '</div>';
                html += '<div class="manga-stat">' + avgMangaScore + '</div>';
                html += '<div>manga read</div>';
                html += '<div>chapters read</div>';
                html += '<div>average score</div>';
            $('#manga-stats').append(html);
        })
        .catch(handleError);
